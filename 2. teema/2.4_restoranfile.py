from Restoran import Restoran
restoran1 = Restoran()
restoran1.restorani_nimi = " Hesburger"
restoran1.soogi_tyyp = " Burgerid"
restoran1.restorani_kirjeldus()
restoran1.ava_restoran()


restoran2 = Restoran()
restoran2.restorani_nimi = " McDonalds"
restoran2.soogi_tyyp = " Burgerid ja friikartuliud"
restoran2.restorani_kirjeldus()
restoran2.ava_restoran()

restoran3 = Restoran()
restoran3.restorani_nimi = " Kartul"
restoran3.soogi_tyyp = " Kartulid nii keededult kui praetult"
restoran3.restorani_kirjeldus()
restoran3.ava_restoran()
